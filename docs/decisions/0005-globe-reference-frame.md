...
- Continental polygons (and their union, `landmass`) mark continental crust, not shorelines; at 240 Ma they cut through the painted land of the PaleoDEM. The globe therefore draws no continent outlines by default; the coastline visible on the globe is the sea-level colour step of the PaleoDEM texture (decided 2026-09-12 after inspecting overlays at 240, 100 and 20 Ma). Amended 2026-09-13: the plate-tectonic chapters (`content/timeline/01`-`04`) opt in to a faint `landmass` outline (opacity 0.28) to give the blurry PaleoDEM texture definition; it is continental crust from the same PALEOMAP model, labelled modeled and cited with the plate model. Block outlines remain an optional detail layer.
